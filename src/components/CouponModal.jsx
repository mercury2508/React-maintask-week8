...
function CouponModal({
    ...
    const addNewCoupon = async () => {
        ...
        try {
            await axios.post(
                `${baseUrl}/api/${apiPath}/admin/coupon`,
                couponData
            );
    ...
    const handleUpdateProduct = async () => {
        const apiCall =
            couponModalState === "add" ? addNewCoupon : adjustCoupon;
        try {
            await apiCall();
            setIsCouponModalOpen(false);
    ...
    const formatDate = (timestamp) => {
        ...
        if (!timestamp){
            return ""
        } else if(isNaN(Number(timestamp))){//若時間戳記為字串則直接回傳字串
            return timestamp;
        }
